...
public class McpServerService {
	...
	public List<Map<String, Object>> listAgentsToolCallback(AgentListRequest agentListRequest) {
		List<Agent> list = agentMapper.findByConditions("published", agentListRequest.keyword());
		return list.stream().map(agent -> {
			Map<String, Object> agentWithDbConfig = new HashMap<>();
			agentWithDbConfig.put("id", agent.getId());
			agentWithDbConfig.put("name", agent.getName());
			agentWithDbConfig.put("description", agent.getDescription());
			return agentWithDbConfig;
		}).toList();
	}
...
//	}
	...
	@Tool(description = "将自然语言查询转换为SQL语句并执行返回结果。使用指定的智能体将用户的自然语言查询描述转换为可执行的SQL语句，执行SQL查询并返回查询结果，支持复杂的数据查询需求。")
	public String nl2QueryDataToolCallback(Nl2SqlRequest nl2SqlRequest) throws Exception {
		Assert.hasText(nl2SqlRequest.agentId(), "AgentId cannot be empty");
		Assert.hasText(nl2SqlRequest.naturalQuery(), "Natural query cannot be empty");

		return graphService.nl2QueryData(nl2SqlRequest.naturalQuery(), nl2SqlRequest.agentId());
	}
